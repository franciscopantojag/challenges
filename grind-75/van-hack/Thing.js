class Thing {
  constructor(name) {
    const define = (name, getValue, target = this, enumerable = false) =>
    ...
    const generateBooleanProperties = (value, thisValue = this) =>
      getProxy({
        get: (_, name) => {
    ...
    const generatePropertyWithValue = (thisValue = this) =>
      getProxy({
        get: (target, name) =>
          (target[name] = getProxy({
            get: (_, nameValue) => {
              define(name, nameValue, thisValue, true);

              return getProxy({
                get: (_, nextPropName) =>
                  nextPropName === 'and_the'
    ...
    const generateHasMethod = (handler) =>
      getProxy(
        {
    ...
    const getSafeFunctionResult = (thisValue, callback, args = []) => {
      const thisPropsKeys = Object.getOwnPropertyNames(thisValue).filter(
        (prop) => thisValue.propertyIsEnumerable(prop)
      );
      const newFn = new Function(
        ...thisPropsKeys,
        `return ${callback.toString()}`
      );
      return newFn.apply(thisValue, [
        ...thisPropsKeys.map((prop) => thisValue[prop]),
      ])(...args);
    };

    const generateNestedItemsCallback = () =>
      getProxy(
        {
          apply: (_, thisValue, args) => {
            const getFnBody = (fnSrt) =>
              `this.${fnSrt
                .toString()
                .replace(/^[^{]*{\s*[return]*/, '')
                .replace(/\s*}[^}]*$/, '')
                ...
                getProxy(
                  {
                    apply: (target, thisValue) =>
                      new Function('h', getFnBody(target)).call(thisValue),
                  },
                  args[0]
                ).call(thing) && thing
            );
    ...
    define(
      'has',
      generateHasMethod((propName, itemsNumber) => {
        const getNestedItem = (propName) => {
          const childThing = new Thing(propName);
          define(
            'with',
            getProxy({}, childThing.having, new Function()),
            childThing
          );
          define(
            'being_the',
            generatePropertyWithValue(childThing),
            childThing
          );
          return childThing;
        };
        ...
        if (itemsNumber > 1) {
          this[propName] = [...Array(itemsNumber)].map(() =>
            getNestedItem(propName.slice(0, -1))
          );
          ...
          getProxy(
            {
              apply: (_, __, args) => {
                ...
                if (argsCopy.length) {
                  argsCopy.forEach((methodName) =>
                    define(methodName, callsHistory)
                  );
                }
                ...
                define(
                  ...
                  getProxy(
                    {
                      apply: (_, thisValue, args) => {
                        const callResult = getSafeFunctionResult(
                          thisValue,
                          callback,
                          args
                        );
